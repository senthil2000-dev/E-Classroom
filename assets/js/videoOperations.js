...
function bookmark(button, videoId) {
    $.post("ajax/bookmarkVideo.php", {videoId: videoId})
    .done(function(data) {
        console.log(data);
        var bookmarkButton=$(button);
        if(data==0) {
            bookmarkButton.find("span").text("BOOKMARKED");
            bookmarkButton.addClass("active");
        }
        else {
            bookmarkButton.find("span").text("BOOKMARK");
